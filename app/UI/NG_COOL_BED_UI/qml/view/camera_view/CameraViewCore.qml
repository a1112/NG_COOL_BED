...
Item {
    ...
    property ListModel layoutConfigs: ListModel {
        ListElement { key: "all"; name: "全部相机 (4x4)"; grid: [[1, null, null, 6],[2,3,4,5],[7,13,null,12],[8,9,10,11]] }
        ListElement { key: "L1";  name: "一号冷床 (2x4)"; grid: [[1, null, null, 6],[2,3,4,5]] }
        ListElement { key: "L2";  name: "二号冷床 (2x4)"; grid: [[7,13,null,12],[8,9,10,11]] }
    }
    ...
    function currentLayoutConfig() {
        const count = layoutConfigs ? layoutConfigs.count : 0
        if (!count) return null
        const idx = Math.min(Math.max(selectedLayoutIndex, 0), count - 1)
        return layoutConfigs.get(idx)
    }
    ...
    function loadCurrentCalibrate() {
        ...
        try {
            if (typeof app_api !== "undefined" && app_api.server_url) {
                const url = app_api.server_url.url(app_api.server_url.serverUrl, "config", "calibrate")
    ...
    function getLabelme(camId) {
        ...
        const url = app_api.server_url.url(app_api.server_url.serverUrl, "calibrate", "label", calibrateKey, camId)
    ...
    function snapshotPath(camId) {
        ...
        if (typeof app_api !== "undefined" && app_api.server_url) {
            return app_api.server_url.url(app_api.server_url.serverUrl, "calibrate", "image", calibrateKey, imgName)
        }
    ...
    function reloadFromServer() {
        ...
        cameraManageData = ({})
        if (typeof app_api !== "undefined" && app_api.server_url) {
            const url = app_api.server_url.url(app_api.server_url.serverUrl, "calibrate", "camera_manage", calibrateKey)
            const data = loadJson(url)
    ...
    function refreshFromApi() {
        if (!app_api || !app_api.get_cameras) {
            buildCameraInfo([])
